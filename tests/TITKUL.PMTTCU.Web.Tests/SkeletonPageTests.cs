...
public sealed class SkeletonPageTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    {
        using var client = _factory.CreateClient(new WebApplicationFactoryClientOptions { AllowAutoRedirect = false });
        foreach (var path in new[] { "/admin/chuong-trinh", "/admin/doi-tuong", "/admin/thon-ap", "/admin/phong-hoc", "/admin/lop-hoc", "/admin/lich-hoc", "/admin/lop-hoc/" + Guid.Empty, "/admin/hoc-vien", "/admin/hoc-vien/" + Guid.Empty })
        {
