...
public sealed class SkeletonPageTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    {
        using var client = _factory.CreateClient(new WebApplicationFactoryClientOptions { AllowAutoRedirect = false });
        foreach (var path in new[] { "/admin/chuong-trinh", "/admin/doi-tuong", "/admin/thon-ap", "/admin/phong-hoc", "/admin/lop-hoc", "/admin/lich-hoc", "/admin/lop-hoc/" + Guid.Empty, "/admin/hoc-vien", "/admin/hoc-vien/" + Guid.Empty, "/admin/diem-danh/" + Guid.Empty, "/admin/lop-hoc/" + Guid.Empty + "/chung-nhan", "/admin/cms/chuyen-muc", "/admin/cms/bai-viet", "/admin/cms/bai-viet/sua" })
        {
    ...
    public async Task Public_class_pages_render()
    {
        ...
        foreach (var path in new[] { "/lop-hoc", "/lop-hoc/ABC12", "/dang-ky/ABC12", "/dang-ky/ABC12/cam-on", "/dang-ky/ABC12/cam-on?code=DKTESTCODE12ABCD", "/diem-danh-buoi/" + Guid.Empty, "/tra-cuu", "/tra-cuu/CNTEST", "/tin-tuc", "/tin-tuc/bai-thu" })
        {
            var response = await client.GetAsync(path);
